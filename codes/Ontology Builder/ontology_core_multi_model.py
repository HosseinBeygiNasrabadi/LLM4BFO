...
# helpers
# ---------------------------
_BYTES_LIT_RX = re.compile(r"""b(['"])(.*?)\1""", re.DOTALL)

_BYTES_TOKEN_RX = re.compile(
    r"""b'(?:\\.|[^'\\])*'|b"(?:\\.|[^"\\])*""" + r"""""" ,
    re.DOTALL,
)

def _extract_join_decode_bytes_literals(s: str) -> str:
    """
    If s contains Python bytes literal tokens (b'...' or b"..."),
    extract ALL of them, literal-eval them to bytes, join, decode to UTF-8.
    This removes corruption like ...<IRI'^b''... which breaks Turtle IRIs.
    """
    s = (s or "").strip()
    toks = _BYTES_TOKEN_RX.findall(s)
    if not toks:
        return s
    ...
    for t in toks:
        try:
            v = ast.literal_eval(t)  # -> bytes
            if isinstance(v, (bytes, bytearray)):
                chunks.append(bytes(v))
        except Exception:
            # if any token is malformed, skip it
            continue
...
def _decode_if_bytes_repr(s: str) -> str:
    """
    Repairs a common failure mode where the model output is wrapped as a Python bytes-literal
    representation, e.g.  b'@prefix ...\\n...'
    Also handles concatenated fragments: b'...' b'...'
    """
    s = (s or "").strip()

    # Case A: whole string is exactly a Python literal like b'...'
    ...
    # Case B: contains multiple b'...' fragments
    frags = [m.group(2) for m in _BYTES_LIT_RX.finditer(s)]
    if frags:
        joined = "".join(frags)
        # interpret escape sequences like \n, \t that appear in repr output
        try:
            joined = codecs.decode(joined, "unicode_escape")
        except Exception:
            pass
        return joined.strip()
...
def _repair_split_prefixed_names(ttl: str) -> str:
    """
    Repairs common LLM wrap error: `prefix:local part2` -> `prefix:localpart2`
    Only when the second token is a plain name token and is followed by Turtle punctuation.
    """
    rx = re.compile(
        r"(\b[A-Za-z][\w-]*:)([A-Za-z0-9_]+)\s+([A-Za-z0-9_]+)(?=\s*[.;,\]\}])"
    )
...
def _strip_code_fences(s: str) -> str:
    ...
    # Remove any fence lines anywhere (``` or ```turtle etc.)
    lines = s.splitlines()
...
def _call_anthropic(api_key: str, model: str, system_text: str, user_prompt: str) -> str:
    ...
    r = requests.post(url, headers=headers, json=payload, timeout=(30, 2400))
    data = r.json()
    parts = []
...
def _call_gemini(api_key: str, model: str, system_text: str, user_prompt: str) -> str:
    ...
    r = requests.post(url, json=payload, timeout=(30, 2400))
...
""".strip()
        ...
        # 1) bytes -> str (if needed)
        if isinstance(raw_text, (bytes, bytearray)):
            raw_text = raw_text.decode("utf-8", errors="replace")

        # 2) if output contains b'...' fragments, reconstruct true text
        raw_text = _extract_join_decode_bytes_literals(raw_text)
