...
def _augment_namespaces_for_obo(local_ns_uris: Set[str]) -> Set[str]:
    out = set(local_ns_uris)
    for ns in list(local_ns_uris):
        if "purl.obolibrary.org/obo" in ns:
            out.add("http://purl.obolibrary.org/obo/")
    return out
...
def _get_local_namespace_uris_pipeline(graph: Graph) -> Set[str]:
    # identical to ontology_core_multi_model._get_local_namespace_uris
...
def select_local_class_nodes(path: str) -> Tuple[Graph, List]:
    """
    ...
    # identical _select_local_classes()
    selected = []
    if local_ns_uris:
    ...
    if not selected:
        prefix = _infer_dominant_id_prefix(class_uris)
        if prefix:
            for cls in class_nodes:
                u = str(cls)
                local = u.rsplit("/", 1)[-1]
    ...
    selected = sorted(selected, key=lambda x: str(x))  # deterministic
    return g, selected
...
def discover_definition_predicates_human(g: Graph) -> List[URIRef]:
    ...
    # keep your existing base + subproperty logic
    for b in bases:
        add(b)
        # ... your subPropertyOf expansion stays the same ...

    # ---- CHANGE: broaden candidates to include predicates used with literals ----
...
def select_local_class_nodes_like_builder(g: Graph, source_path: str) -> List[URIRef]:
    """
    SAME selection logic as builder's load_rdf_terms_with_hierarchy():
      1) filter by derived local namespaces (plus xml:base for OWL)
      2) fallback: dominant OBO-style ID prefix (e.g., PROV_O_1234)
      3) fallback: all owl:Class
    """
    ...
    # 3) fallback: all classes
    return sorted(class_nodes, key=lambda x: str(x))
...
def first_bfo_from_graph(graph: Graph, class_label: str) -> Optional[str]:
    ...
    """
    for cls in graph.subjects(RDF.type, OWL.Class):
        if best_label(graph, cls).strip().lower() == class_label.strip().lower():
            start_nodes.append(cls)
    """
...
# =================================================
# GROUND-TRUTH CLASS EXTRACTION ONLY
# =================================================
def _guess_namespace(uri: str) -> str:
    i_hash = uri.rfind("#")
    i_slash = uri.rfind("/")
...
def extract_local_classes_from_path(path: str) -> pd.DataFrame:
    ...
    # Use closure graph for BFO traversal
    g_closure = parse_rdf_graph_closure(path, import_map=None)  # or pass a local mapping
    ...
    for cls in nodes:
        ...
        lbl = label_like_pipeline(g_local, cls)  # keep label source stable
...
def label_match_eval(df_gt, df_pred, threshold=0.70):
    # keep row identity (important if there are duplicates)
...
def definition_similarity(df_matches, human_ttl, model_ttl):
    ...
    def load_defs_model(path):
        ...
        for cls in g.subjects(RDF.type, OWL.Class):
            ...
            defs = []
            # KEEP EXACTLY your prior LLM TTL extraction source:
